...
const tips1 =[
 "使用高级代理将获得更多请求",
 "该脚本只适于HTTP/2目标",
 "如果目标为 Akamai/Fastly 建议使用更高级的代理文件"
];
const platformd = [
 "Windows",
 "Linux",
...
const RESTART_DELAY = 1000;
 ...
 if (cluster.isMaster) {
    console.clear()
    console.log(`Cloudflare Bypass`.rainbow)
    console.log(`--------------------------------------------`.gray)
    ...
    console.log(`文件：`.brightYellow + process.argv[6])
    console.log(`--------------------------------------------`.gray)
    console.log(`注意：`.brightCyan + tipsz)
    ...
    const handleRAMUsage = () => {
        ...
        if (ramPercentage >= MAX_RAM_PERCENTAGE) {
            console.log('[!] 最大 RAM 使用率百分比：', ramPercentage.toFixed(2), '%');
            restartScript();
        }
    };
